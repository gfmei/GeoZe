...
from semseg.semmodel.disnet import get_model
from semseg.semmodel.semgeoze import SemGeoze
...
from semseg.utils.util import get_palette, extract_text_feature, export_pointcloud, convert_labels_with_palette, \
    visualize_labels
...
def evaluate(model, val_data_loader, labelset_name='scannet_3d'):
    ...
    transformer = SemGeoze(dim=768, sigma_d=0.01, sigma_a=0.001, sigma_e=0.001, angle_k=10,
                                voxel_size=args.voxel_size, n_pts=2000).cuda()
    transformer.eval()
    ...
    with torch.no_grad():
        ...
        for rep_i in range(args.test_repeats):
            ...
            for i, (coords, xyz, feat, label, feat_3d, spts_idx, mask, inds_reverse) in enumerate(tqdm(val_data_loader)):
                ...
                elif feature_type == 'attn':
                    new_feat_3d = feat_3d.cuda(non_blocking=True)
                    feat = feat.cuda(non_blocking=True)
                    xyz = xyz.cuda(non_blocking=True)
                    new_feat_3d = transformer(xyz, feat[:, 3:6], feat[:, 6:], new_feat_3d, spts_idx)
                    predictions = new_feat_3d[inds_reverse, :]
                    pred = predictions.half() @ text_features.t()
                    logits_pred = torch.max(pred, 1)[1].detach().cpu()
                    if mark_no_feature_to_unknown:
                        # some points do not have 2D features from 2D feature fusion.
                ...
                if vis_pred:
                    if mapper is not None:
                        ...
                        export_pointcloud(os.path.join(save_folder, '{}_{}.ply'.format(i, feature_type)), pcl,
                ...
                if vis_gt:
                    ...
                    if 'nuscenes' in labelset_name:
                        all_digits = np.unique(
                            np.concatenate([np.unique(mapper[logits_pred].numpy()), np.unique(label)]))
                        labelset = list(NUSCENES_LABELS_16)
                        labelset[4] = 'construct. vehicle'
                        labelset[10] = 'road'
                        visualize_labels(list(all_digits), labelset,
                                         palette, os.path.join(save_folder, '{}_label.jpg'.format(i)),

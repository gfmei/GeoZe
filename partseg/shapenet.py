...
cat2part = {'airplane': ['body', 'wing', 'tail', 'engine or frame'], 'bag': ['handle', 'body'],
            'cap': ['panels or crown', 'visor or peak'],
            'car': ['roof', 'hood', 'wheel or tire', 'body'],
            'chair': ['back', 'seat pad', 'leg', 'armrest'], 'earphone': ['earcup', 'headband', 'semdata wire'],
            'guitar': ['head or tuners', 'neck', 'body'],
            'knife': ['blade', 'handle'], 'lamp': ['leg or wire', 'lampshade'],
            'laptop': ['keyboard', 'screen or monitor'],
            'motorbike': ['gas tank', 'seat', 'wheel', 'handles or handlebars', 'light', 'engine or frame'],
            'mug': ['handle', 'cup'],
            'pistol': ['barrel', 'handle', 'trigger and guard'],
            'rocket': ['body', 'fin', 'nose cone'], 'skateboard': ['wheel', 'deck', 'belt for foot'],
            'table': ['desktop', 'leg or support', 'drawer']}
id2part2cat = [['body', 'airplane'], ['wing', 'airplane'], ['tail', 'airplane'], ['engine or frame', 'airplane'],
               ['handle', 'bag'], ['body', 'bag'],
               ...
               ['earcup', 'earphone'], ['headband', 'earphone'], ['semdata wire', 'earphone'],
               ['head or tuners', 'guitar'], ['neck', 'guitar'], ['body', 'guitar'], ['blade', 'knife'],
               ['handle', 'knife'],
               ['support or tube of wire', 'lamp'], ['lampshade', 'lamp'], ['canopy', 'lamp'],
               ['support or tube of wire', 'lamp'],
               ['keyboard', 'laptop'], ['screen or monitor', 'laptop'], ['gas tank', 'motorbike'],
               ['seat', 'motorbike'], ['wheel', 'motorbike'],
               ['handles or handlebars', 'motorbike'], ['light', 'motorbike'], ['engine or frame', 'motorbike'],
               ['handle', 'mug'], ['cup or body', 'mug'],
               ['barrel', 'pistol'], ['handle', 'pistol'], ['trigger and guard', 'pistol'], ['body', 'rocket'],
               ['fin', 'rocket'], ['nose cone', 'rocket'],
...
def load_data_partseg(data_path, partition):
    ...
    if partition == 'trainval':
        file = (glob.glob(os.path.join(data_path, 'shapenet_part_seg_hdf5_data', '*train*.h5'))
                + glob.glob(os.path.join(data_path, 'shapenet_part_seg_hdf5_data', '*val*.h5')))
    elif partition == 'train':
        file = glob.glob(os.path.join(data_path, 'shapenet_part_seg_hdf5_data', '*train*.h5'))
    elif partition == 'val':
        file = glob.glob(os.path.join(data_path, 'shapenet_part_seg_hdf5_data', '*val*.h5'))
    else:
        file = glob.glob(os.path.join(data_path, 'shapenet_part_seg_hdf5_data', '*test*.h5'))
    for h5_name in file:
